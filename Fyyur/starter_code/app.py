...
def search_venues():
    ...
    for result in results:
        id=db.session.query(Venue.id).filter_by(name=result[0]).all()[0][0]
        data.append({
        "id":id,
        "name":result[0],
        "num_upcoming_shows": len(db.session.query(Show).filter_by(id=id).all()),
        })
...
def show_venue(venue_id):
  ...
  show = Show.query.filter_by(venue=venue_id).all()
  past_show = []
  upcoming_show = []
  for ps in show:
      if ps.start_time < datetime.now():
          past_show.append(
            {
            "artist_id": ps.artist,
            "artist_name": Artist.query.get(ps.artist).name,
            "artist_image_link": Artist.query.get(ps.artist).image_link,
